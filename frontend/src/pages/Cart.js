...
export default function Cart() {
	...
	useEffect(() => {
        ...
        const fetchCartItems = async () => {
            try {
                const response = await axios.get(`http://15.235.155.26:5000/api/cart-items/${userEmail}`); // Thay bằng email của người dùng đăng nhập
    ...
    const handleRemoveItem = async (product) => {
        try {
			await axios.delete(`http://15.235.155.26:5000/api/delete-cart-items/${product.id}`);
    ...
    const handleOrder = async () => {
        try {
            const response = await axios.post('http://15.235.155.26:5000/api/order', { email: userEmail });
