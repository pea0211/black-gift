...
export default function LsRutTien() {
  ...
  useEffect(() => {	
    ...
    const fetchTransactions = async () => {
      try {
        const response = await axios.get(`http://15.235.155.26:5000/api/ls-rut-tien/${userEmail}`);
        console.log(response.data);
  ...
  const columns = [
    { title: 'Mã rút tiền', dataIndex: 'id', key: 'id' },
    { title: 'Ngày rút', dataIndex: 'date', key: 'date' },
    { title: 'Ngân hàng', dataIndex: 'bank', key: 'bank' },
    { title: 'Số tài khoản', dataIndex: 'acc_number', key: 'acc_number' },
    { title: 'Chủ tài khoản', dataIndex: 'owner', key: 'owner' },
    ...
    },
    {
      ...
      key: 'status',
      render: (text) => (
        <span className={`tag tag-${text === 'Thành công' ? 'completed' : text === 'Đang xử lý' ? 'processing' : 'pending'}`}>
          {text}
        </span>
      ),
    },
        ...
        <div className="container">
          <div className="banner_text">
            ...
            <ul>
              ...
              <li><Link to="/ls-rut-tien">Rút tiền</Link></li>
            </ul>
          </div>
        </div>
      </section>
