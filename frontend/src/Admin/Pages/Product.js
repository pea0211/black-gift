...
export default function Product() {
    ...
    useEffect(() => {
        const fetchProducts = async () => {
          try {
            const response = await axios.get(`http://15.235.155.26:5000/api/admin/giftbox`);
    ...
    const handleSaveProduct = async (e) => {
        ...
        try {
          const response = await axios.post('http://15.235.155.26:5000/api/new-giftbox', formData, {
            headers: {
    ...
    const handleOpenEditModal = (product) => {
        ...
        setImageURL(product.image ? `http://15.235.155.26:5000/api${product.image}` : null);
    ...
    const handleEditProduct = async () => {
        ...
        try {
            const response = await axios.post(`http://15.235.155.26:5000/api/edit-giftbox/${editingProductId}`, formData, {
                headers: {
                    'Content-Type': 'multipart/form-data',
                },
            });
    ...
    const handleDelete = async (id) => {
        if (window.confirm('Bạn có chắc chắn muốn xóa hộp quà này?')) {
            try {
                const response = await axios.delete(`http://15.235.155.26:5000/api/delete-giftbox/${id}`);
                alert(response.data.message);
