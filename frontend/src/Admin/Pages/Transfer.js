...
export default function Transfer() {
  ...
  useEffect(() => {	
    const fetchTransactions = async () => {
      try {
        const response = await axios.get(`http://localhost:5000/admin/nap-tien`);
  ...
  const handleUpdate = async () => {
    try {
      ...
      const response = await axios.post(`http://localhost:5000/admin/update-nap-tien/${selectedTransaction.in_code}`, {
        status: updatedStatus,
      });
